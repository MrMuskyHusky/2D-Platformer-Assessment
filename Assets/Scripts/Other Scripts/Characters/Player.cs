...
public class Player : MonoBehaviour
{
    ...
    public float portalDistance = 1f; // Hpw far from the Portal the player needs to be to press Up and Interact
    ...
    private void OnTriggerEnter2D(Collider2D col)
    {
        ...
        {
            // Add 1 to score - Requires a GameManager
            GameManager.Instance.AddScore(1);
            // Play chim sound - Requires an Audio Source
    ...
    {
        // Detect hitting protal
        if (col.CompareTag("Portal"))
        {
            // Store the current protal
            currentPortal = null;
        }
    }
